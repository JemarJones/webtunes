...
s=(r-e)%2===0?"even":"odd"
var l=$('<tr data-num="'+r+'" data-id="'+a[r].track_id+'" class="song '+s+'"></tr>')
l.append($('<td class="title">'+a[r].title+"</td>")),l.append($('<td class="album">'+a[r].album+"</td>")),l.append($('<td class="artist">'+a[r].artist+"</td>")),l.append($('<td class="playcount">'+a[r].playcount+"</td>"))
var i
i="-"==a[r].track_id?"notplayable":"playable",l.append($('<td class="'+i+'"></td>')),$("#songView").append(l)}$(".song").on("click",expandSong)},expandAlbum=function(){if(!expanded){expanded=!0
for(var a=this,t=albums[$(a).data("albumnum")],e="https://embed.spotify.com/?uri=spotify:trackset:"+t[0].album+":",r=0;r<t.length;r++)e+=r==t.length-1?t[r].track_id:t[r].track_id+",",$("#trackList").append('<li class="track hover" data-id="'+t[r].track_id+'"data-title="'+t[r].title+'"data-artist="'+t[r].artist+'" data-num='+r+'><img class="trkImg" src="../images/play_button.png"/>  '+t[r].title+" - "+t[r].artist+"</li>")
$("#albWrapper").addClass("hover"),displayPlayer([t[0].art_lg,t[0].album,t[0].artist],$("#albumView")),$("#albWrapper").attr("data-ids",e),$(".track").on("click",playTrack)}},closePlayer=function(){$("#bg").fadeOut(),$("#overlay").fadeOut(400,function(){$("#header").fadeIn(),viewToRestore.fadeIn(),$(".track").remove(),$("iframe").remove()}),expanded=!1},expandSong=function(){if(!expanded){expanded=!0
var a=parseInt($(this).attr("data-num"),10)
if(displayPlayer([songs[a].art_lg,songs[a].title,songs[a].artist],$("#songView")),$("#albWrapper").removeClass("hover"),"-"!==$(this).attr("data-id")){var t="https://embed.spotify.com/?uri=spotify:trackset:"+$("#songAlbum").text()+":"+$(this).attr("data-id")
$("iframe").remove()
var e=$('<iframe frameborder="0" allowtransparency="true" src="'+t+'"></iframe>')
$("#overlay").append(e)}}},displayPlayer=function(a,t){viewToRestore=t,""!==a[0]&&($("#bg").css("background-image","url("+a[0]+")"),$("#bigAlb").attr("src",a[0])),$("#songAlbum").text(a[1]),$("#songArtist").text(a[2]),$("#header").fadeOut(),viewToRestore.fadeOut(400,function(){$("#bg").fadeIn(),$("#overlay").fadeIn()})},playTrack=function(){if(displayPlayer(["",$(this).attr("data-title"),$(this).attr("data-artist")],$("#albumView")),$(".track").fadeOut(),"-"!=$(this).attr("data-id")){var a="https://embed.spotify.com/?uri=spotify:trackset:"+$(this).attr("data-title")+":"+$(this).attr("data-id")
$("iframe").remove()
...
var a=$('<iframe frameborder="0" allowtransparency="true" src="'+$(this).attr("data-ids")+'"></iframe>')
$("#overlay").append(a)}
